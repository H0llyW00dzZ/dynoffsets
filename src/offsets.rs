...
pub fn discover_globals() -> RuntimeGlobals {
    ...
    let dw_csgo_input =
        find_offset_rip32(&client, sig!(0x48 0x89 0x05 ? ? ? ? 0x0F 0x57 0xC0 0x0F 0x11 0x05), 3);
    ...
    // cs2-dumper: pattern!("488b1d${'} 4532f6") — was 48 8B 15 … 41 FF C0.
    let dw_planted_c4 = find_offset_rip32(&client, sig!(0x48 0x8B 0x1D ? ? ? ? 0x45 0x32 0xF6), 3);
    ...
    let dw_local_player_pawn = dw_prediction.and_then(|prediction| {
        find_offset_u32(&client, sig!(0x4C 0x39 0xB6 ? ? ? ? 0x74 ? 0x44 0x88 0xBE), 3)
    ...
    let dw_view_render =
        find_offset_rip32(&client, sig!(0x48 0x89 0x05 ? ? ? ? 0x48 0x8B 0xC8 0x48 0x85 0xC0), 3);
    ...
    let dw_network_game_client_delta_tick =
        find_offset_u32(&engine2, sig!(0x4C 0x8D 0xB7 ? ? ? ? 0x4C 0x89 0x7C 0x24), 3);
    // cs2-dumper: ends with `48 83 EC`, not `40 53`.
    let dw_network_game_client_is_background_map = find_offset_u32(
        &engine2,
        sig!(0x0F 0xB6 0x81 ? ? ? ? 0xC3 0xCC 0xCC 0xCC 0xCC 0xCC 0xCC 0xCC 0xCC 0x0F 0xB6 0x81 ? ? ? ? 0xC3 0xCC 0xCC 0xCC 0xCC 0xCC 0xCC 0xCC 0xCC 0x48 0x83 0xEC),
        3,
    );
    ...
    // cs2-dumper: pattern!("488d0d${'} e8${} 488b0d${} [3] 4c8b82")
    let dw_sound_system = find_offset_rip32(
        &soundsystem,
        sig!(0x48 0x8D 0x0D ? ? ? ? 0xE8 ? ? ? ? 0x48 0x8B 0x0D ? ? ? ? ? ? ? 0x4C 0x8B 0x82),
        3,
    );
    let dw_sound_system_engine_view_data =
        find_offset_u8(&soundsystem, sig!(0x0F 0x11 0x47 ? 0x0F 0x10 0x4E ? 0x0F 0x11 0x8F), 3);
