...
impl Default for Args {
    fn default() -> Self {
        Args { dll: "client.dll".to_string(), enabled: true, static_storage: false }
    }
}
...
mod tests {
    ...
    fn rewrite_schema_disabled_emits_literal_fn() {
        let mut m = parse_mod("mod C_Foo { pub const m_x: usize = 0x10; }");
        let args = Args { dll: "client.dll".into(), enabled: false, static_storage: false };
    ...
    fn static_args(dll: &str) -> Args {
        Args { dll: dll.into(), enabled: true, static_storage: true }
    }
