    ...
    rewrite_dynamic_consts(items, |info| {
        ...
        if enabled {
            cached_accessor(
                ...
                lit_expr,
                quote! {
                    ::dynoffsets::try_lookup_offset(#dll, #class_lit, #field_str)
                },
            )
        } else {
            quote! {
...
fn rewrite_globals_module(module: &mut ItemMod, args: &Args) {
    ...
    rewrite_dynamic_consts(items, |info| {
        ...
        cached_accessor(
            ...
            lit_expr,
            quote! {
                ::dynoffsets::get_runtime_globals().and_then(|g| g.#fn_name)
            },
        )
    });
}
...
/// Emits a `#[inline]` accessor whose first successful invocation resolves
/// `lookup_expr` (an `Option<usize>` expression), latches the live value into
/// per-accessor `AtomicUsize` + `AtomicBool` slots, and from then on returns
/// it via a single relaxed atomic load.
///
/// On miss (pre-`init`, runtime feature off, or the live lookup itself returned
/// `None`) we return `lit_expr` *without* caching, so the next call retries.
/// This avoids permanently latching the literal fallback when the runtime
/// state simply isn't ready yet (e.g. the schema system hasn't been populated
/// before the first accessor fires).
fn cached_accessor(
    vis: &syn::Visibility,
    fn_name: &Ident,
    lit_expr: &TokenStream2,
    ...
    quote! {
        ...
        #vis fn #fn_name() -> usize {
            static CACHE: ::core::sync::atomic::AtomicUsize =
                ::core::sync::atomic::AtomicUsize::new(0);
            static RESOLVED: ::core::sync::atomic::AtomicBool =
            ...
            fn __dynoffsets_resolve() -> usize {
                ...
                let resolved: ::core::option::Option<usize> = #lookup_expr;
                match resolved {
                    ::core::option::Option::Some(v) => {
                        CACHE.store(v, ::core::sync::atomic::Ordering::Relaxed);
                        RESOLVED.store(true, ::core::sync::atomic::Ordering::Release);
                        v
                    }
                    ::core::option::Option::None => #lit_expr,
                }
            }

            if RESOLVED.load(::core::sync::atomic::Ordering::Acquire) {
                CACHE.load(::core::sync::atomic::Ordering::Relaxed)
            } else {
...
mod tests {
    ...
    fn rewrite_schema_enabled_emits_lookup_call() {
        ...
        assert!(s.contains("try_lookup_offset"));
        assert!(s.contains("\"C_Foo\""));
        assert!(s.contains("\"m_x\""));
        // Per-accessor cache machinery is in place.
        assert!(s.contains("AtomicUsize"));
        assert!(s.contains("AtomicBool"));
    ...
    fn rewrite_schema_skips_non_pub_usize_items() {
        ...
        let count = s.matches("try_lookup_offset").count();
        assert_eq!(count, 1);
    }
